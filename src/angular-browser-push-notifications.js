    ...
    this.$get = ['$q', '$log', 'BrowserPushNotificationsStatus', '$rootScope', function($q, $log, BrowserPushNotificationsStatus, $rootScope) {
      ...
      var service = {
        ...
        _getServiceWorkerRegistration: function() {
          return navigator.serviceWorker.ready.catch(
        ...
        register: function() {
          var q = $q.defer();
          return service._workersSupported().then(function() {
        ...
        subscribe: function(serviceWorkerRegistration) {
          var q = $q.defer();
          serviceWorkerRegistration.pushManager.subscribe({userVisibleOnly: userVisibleOnly})
            .then(function(sub) {
              $rootScope.$apply(q.resolve.bind(q, sub));
            }, function(reason) {
              $rootScope.$apply(q.reject.bind(q, BrowserPushNotificationsStatus.FAILED_TO_SUBSCRIBE));
            });

          return q.promise;
        },
        workersSupported: function() {
          return 'serviceWorker' in navigator ? $q.resolve() : $q.reject(BrowserPushNotificationsStatus.WORKERS_NOT_SUPPORTED);
        }
      };
